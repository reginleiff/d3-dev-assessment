...
class Database {
  ...
  public async getTeacher(email: string): Promise<any> {
    ...
    if (result.length == 0)
      throw new Error(Constants.ERR_TEACHER_DOES_NOT_EXIST);
    return result[0];
  }
  ...
  public async getTeacherCount(email: string): Promise<number> {
    const queryString =
      'SELECT COUNT(id) as count FROM teacher WHERE `email` = ?;';
    const [result] = await this.query(queryString, [email]);
  ...
  public async getStudent(email: string): Promise<any> {
    ...
    const [result] = await this.query(queryString, [email]);
    if (result.length == 0)
      throw new Error(Constants.ERR_STUDENT_DOES_NOT_EXIST);
    return result[0];
  }
  ...
  public async getStudentCount(email: string): Promise<number> {
    const queryString =
      'SELECT COUNT(id) as count FROM `student` WHERE `email` = ?;';
    const [result] = await this.query(queryString, [email]);
    const { count } = result[0];
    return count;
  }
  ...
  public async getRegistration(
    ...
    const queryString =
      'SELECT * FROM `registration` WHERE `student_id` = ? AND `teacher_id` = ?;';
    const [result] = await this.query(queryString, [studentID, teacherID]);
    if (result.length == 0)
      throw new Error(Constants.ERR_REGISTRATION_DOES_NOT_EXIST);
    return result[0];
  }
  ...
  public async getRegistrationCount(
    ...
    const queryString =
      'SELECT COUNT(*) as count FROM `registration` WHERE `student_id` = ? AND `teacher_id` = ?;';
    const [result] = await this.query(queryString, [studentID, teacherID]);
    const { count } = result[0];
    return count;
  }

  public async doesRegistrationExist(
    studentID: number,
    teacherID: number
  ): Promise<any> {
